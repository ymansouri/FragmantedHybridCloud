        ...
        do
	           ...
	           if [[ ${SOUR_PRIVATE_IP_INDEX} != ${DEST_PRIVATE_IP_INDEX}  ]]
                   then
                        ...
                        CMD=${CMD}" ; sudo /usr/sbin/tc class add dev ${NETWORK_INTERFACE} parent ${ROOT_HANDLE}: classid ${ROOT_HANDLE}:${RULE_INDEX} htb rate ${BANDWIDTH}Mbit ceil ${BANDWIDTH}Mbit burst ${BANDWIDTH}Mbit cburst ${BANDWIDTH}Mbit ; sudo /usr/sbin/tc qdisc add dev ${NETWORK_INTERFACE} parent ${ROOT_HANDLE}:${RULE_INDEX} handle ${HANDLE_INDEX}: netem  ; sudo /usr/sbin/tc filter add dev ${NETWORK_INTERFACE} protocol ip parent ${ROOT_HANDLE}: prio 5 u32 match ip dst ${DEST_VM_IP}/32 match ip src 0.0.0.0/0 flowid ${ROOT_HANDLE}:${RULE_INDEX}"
                        #echo ${CMD} >>tc-test
                   fi
        done
