...
do
        ...
        ssh -o StrictHostKeyChecking=no -i $SSH_KEY_FILE $VM_USERNAME@${PUBLIC_IP} "rm -f tc-command-file ; sudo tcset ${NETWORK_INTERFACE} --rate 1Mbit/s --delay 1ms --network 0.0.0.0 --tc-command | head -n 2 >> tc-command-file"
        ...
        do
	           ...
	           if [[ $SOUR_PRIVATE_IP_INDEX != $DEST_PRIVATE_IP_INDEX  ]]
                   then
                       ...
                       ssh  -o StrictHostKeyChecking=no -i $SSH_KEY_FILE $VM_USERNAME@${PUBLIC_IP} "sudo tcset ${NETWORK_INTERFACE} --rate $(get_bandwidth ${SOUR_PRIVATE_IP_INDEX} ${DEST_PRIVATE_IP_INDEX})Mbit/s --delay $(get_latency ${SOUR_PRIVATE_IP_INDEX} ${DEST_PRIVATE_IP_INDEX})ms --network ${DEST_PRIVATE_IP} --tc-command --add | tail -n +3 >>tc-command-file"
